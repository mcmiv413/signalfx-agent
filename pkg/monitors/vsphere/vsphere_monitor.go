...
func (vsm *vSphereMonitor) getNumSamplesReqd() int32 {
	return vsm.getNumSamplesReqdSince(vsm.latestPointRetrievalTime)
}
...
func (vsm *vSphereMonitor) getNumSamplesReqdSince(lastTime time.Time) int32 {
	if lastTime.IsZero() {
		return 1
	}
	fSecondsSinceLastInterval := time.Since(lastTime).Seconds()
